...
2. Read the current pipeline state and artifacts before making changes:
   ...
   - `pipeline/01-tina-blueprint.json` if present
   - `pipeline/02-asset-manifest.json` if present
   - `pipeline/03-tina-coverage.json` if present
...
3. Interpret the change request and choose the smallest sufficient rerun scope:
   ...
   - content model / IA / page-structure changes → invalidate `2_6_tina_blueprint` and every downstream phase; regenerate `pipeline/01-tina-blueprint.json`
   - Branding / colors / fonts / image direction → rerun from Phase 3
   - Media-field changes → invalidate `2_6_tina_blueprint`, Phase 3.5/3.6 assets, Phase 4.1 frontend codegen, and coverage
   - Layout / component / frontend implementation changes → rerun from Phase 4
   - Tina coverage changes → rerun Phase 4.1 and rewrite `pipeline/03-tina-coverage.json`
...
5. Mark changed and downstream phases in `pipeline/00-pipeline-state.json` as `invalidated` or `stale`, recording `invalidated_by`, `invalidated_at`, and `rerun_from`.
6. Validate changed artifacts before rerunning phases.
7. Resume the orchestrator from the earliest affected incomplete phase. Do not wipe completed work unless the user explicitly asks for a clean restart.
8. Run the relevant validations and deploy path for the current profile.
9. Finish with a concise summary of what changed, which phases reran, where the project lives locally, and the resulting live URL or blocker.
